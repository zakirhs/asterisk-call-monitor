...
public class TwoPartiesCallsJPanel extends JPanel implements PresentationModel.Listener{
	
	private static final long serialVersionUID = -472674738115620894L;

	private static Logger logger = Logger.getLogger(TwoPartiesCallsJPanel.class);

	private JButton monitorButton = new JButton("monitor") ;
	private JButton dropButton = new JButton("drop");
	private JTable table;
	private Provider provider;
	private TwoPartiesTableModel twoPartiesTableModel;
	
	public TwoPartiesCallsJPanel(Provider provider){
		this.provider = provider;
		
		setLayout(new BorderLayout());
		setBackground(Color.green);
		
		twoPartiesTableModel = new TwoPartiesTableModel(); 
		table = new JTable(twoPartiesTableModel);
		...
		logger.info("TwoPartiesCallsJPanel " + this);
...
//		provider.drop(callId);
	...
	public void fireDataChanged(PresentationModel presentationModel,Call call,int type,boolean attached) {
		...
		if(twoPartiesTableModel.getRowCount() == 1 && attached){
			...
			twoPartiesTableModel.setValueAt("", 0, 0);
			twoPartiesTableModel.setValueAt("", 0, 1);
			twoPartiesTableModel.setValueAt("", 0, 2);
			twoPartiesTableModel.setValueAt("", 0, 3);			
    ...
    class TwoPartiesTableModel extends AbstractTableModel {
