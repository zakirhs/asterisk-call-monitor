...
public class ConferenceCallsJPanel extends JPanel implements PresentationModel.Listener{

	private static Logger logger = Logger.getLogger(ConferenceCallsJPanel.class);
	...
	private ConferenceCallTableModel conferenceCallTableModel;
	
	public ConferenceCallsJPanel(Provider provider){
		this.provider = provider;
		setLayout(new BorderLayout());
		setBackground(Color.red);
		
		conferenceCallTableModel = new ConferenceCallTableModel();
		table = new JTable(conferenceCallTableModel);
		...
		logger.info("ConferenceCallsJPanel " + this);
...
//		provider.drop(callId);
	...
	public void fireDataChanged(PresentationModel presentationModel,Call call,int type,boolean attached) {
		...
		if(conferenceCallTableModel.getRowCount() == 1 && attached){
			...
			conferenceCallTableModel.setValueAt("", 0, 0);
			conferenceCallTableModel.setValueAt("", 0, 1);
			conferenceCallTableModel.setValueAt("", 0, 2);
			conferenceCallTableModel.setValueAt("", 0, 3);			
    ...
    class ConferenceCallTableModel extends AbstractTableModel {
    	private static final long serialVersionUID = 1L;
    	private Logger log = Logger.getLogger(ConferenceCallTableModel.class);
    	
		private String[] columnNames = { "ID", "Participants", "State",
				"Reason", };
